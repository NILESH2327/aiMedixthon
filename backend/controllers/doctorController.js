...
//Helper function
//this function will convert time in "hh:mm AM/PM" format to total minutes from midnight, which will help in sorting the schedule slots correctly.
...
// this function will remove duplicate time slots for each date and sort them in chronological order.
...
// this function will parse the schedule input which can be either a JSON string or an object, and then it will deduplicate and sort the schedule slots for each date.
...
// this function will normalize the doctor document before sending it to the client, it will convert the Mongoose Map to a plain object and also set default values for some fields if they are missing.
...
export async function createDoctor(req, res) {
   try{
      ...
      if(!body.email || !body.password || !body.name){
        return res.status(400).json({error: "Missing required fields: email, password, name",success: false});
      }
      const emailLC = (body.email|| "").toLowerCase();

      if(await Doctor.findOne({email: emailLC})){
        return res.status(400).json({error: "Doctor with this email already exists",success: false});
      }
    ...
    if(!secret){
      console.warn("JWT secret is not configured in the environment variables");
      return res.status(500).json({error: "server missconfigured",success: false});  
    }
    ...
    delete out.password; // remove password from the output
...
export const getDoctors = async (req, res) => {
  try {
    ...
    const total = await Doctor.countDocuments(match);// total number of doctors matching the search criteria, used for pagination metadata
...
// to get doctor by id ie to fetch one doctor details 
export async function getDoctorById(req, res) {
  try {
    ...
    console.error("getDoctorById err:", error);
    return res.status(500).json({ error: "Internal server error", success: false });
    }
}
...
// to toggle the availability of a doctor
...
// to login a doctor and generate a JWT token for authentication
