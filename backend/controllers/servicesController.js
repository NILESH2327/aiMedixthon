...
// this fxn takes date time slot strings and grp them into a // yyyy-mm-dd with the time 
...
export async function createService(req,res){
    ...
    if (req.file) {
     ...
     return res.status(201).json({
      ...
      console.error("created server error")
...
export async function getServiceById(req,res){
    ...
    try{
      ...
      console.error("get  service id error");
      return res.status(500).json({
        success:false,
        message :"service error",
     }); 
...
export async function updateSevice(req,res){
  ...
  try{
    ...
    return res.status(200).json({
     ...
     console.error("update service id error");
      return res.status(500).json({
        success:false,
        message :"service error",
     }); 
...
// to delete a perticular a service ..
export async function deleteService(req,res){
  ...
  try{
     ...
     if(!existing)return res.status(404)(
      ...
      if(existing.imagePublicId) {
        try{
           ...
           console.warn("field to delete image from clodinary",
             err?.message || err 
            );
        }
      }
     ...
     console.error("delete service id error");
      return res.status(500).json({
        success:false,
        message :"service error",
     }); 
