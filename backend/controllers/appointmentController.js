...
export const getAppointmentsByPatient = async(req,res)=>{
 ...
 console.log("createdBy query:", req.query.createdBy);
 ...
 console.log("resolvedCreatedBy:", resolvedCreatedBy);
    ...
    const appointments = await Appointment.find(filter).sort({date:1,time:1}).lean();
     return res.json({success:true,appointments});
      ...
      console.error("GetAPPointmentbypateint error",err);
      return res.status(500).json({
        success:true,
        message :"server error",
      })
...
export const updateAppointment = async (req,res)=>{
  try{
    ...
    return res.json({success :true ,appointment: updated});
     ...
     console.log("updated appointment" ,err);
...
export const cancelAppointment = async (req,res)=>{
  ...
  try{
    ...
    console.log("canceled appointment" ,err);
...
export const getStats = async(req,res)=>{
  try{
      const total = await Appointment.coundDocuments();
...
export const getAppointmentsByDoctor = async(req,res)=>{
  try{
    ...
    const items = await Appointment.find(filter).sort({date:1,time:1}).skip(skip).limit(limit).populate("doctorId" , "name specialization ownerr image Url image").lean();

    const total = await Appointment.countDocuments(filter);
    return res.json({
        success:true,
        appointments :items,
        meta: {page,limit, total,coutn : items.length}
    });
