...
const stripe = stripeKey
  ? new Stripe(stripeKey, {
      apiVersion: "2026-06-24.dahlia",//2022-11-15
    })
  : null;
...
function resolveClerkUserId(req) {
  try {
    ...
    const auth = req.auth || {};
    const candidate = auth?.userId || auth?.user_id || auth?.user?.id || req.user?.id || req.userId || null;
...
export const createServiceAppointment = async(req,res)=>{
  try{
    ...
    if(!clerkUserId) return res.status(401).json({
      success:false,
      message:"auth is req to create a service appointment"
    })
...
//to get ServiceAppointmentBy id 
...
//to update an appointment by id
...
export const getSerivcesAppointmentsByPateint = async(req,res)=>{
  try{
     const clerkUserId= resolveClerkUserId(req);
    ...
    console.error("getSerivcesAppointmentsByPateint error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export default {
  ...
  getSerivcesAppointmentsByPateint,
