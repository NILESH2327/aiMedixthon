...
function parseDateTime(dateStr, timeStr) {
  ...
  if (parts.length === 3) {
    ...
    return new Date(Number(y), months[m], Number(d), hour, minute, 0);
  }
  ...
  if (itemStatus === "Pending")
    return (
      <span className={badgeStyles.statusBadge.pending}>
        <Clock
         className={iconSize.small} /> Pending
      </span>
    );
...
<h2 className={appointmentPageStyles.serviceTitle}>
    ...
    {!loadingServices && appointmentData.length === 0 && (
