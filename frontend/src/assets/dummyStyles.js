...
export const bannerStyles = {
  ...
  // Main container — flat card, no spinning rainbow border.
  // A single hairline in the brand teal reads as "clinical", not "template".
  mainContainer: "relative rounded-[28px] bg-white shadow-[0_1px_2px_rgba(11,43,34,0.04),0_20px_40px_-24px_rgba(11,43,34,0.25)] ring-1 ring-[#0E7C66]/15 overflow-hidden",

  // Border outline styles — kept as keys for compatibility, now just a
  // quiet corner accent instead of an animated conic-gradient band.
  borderOutline: "absolute inset-0 rounded-[28px] pointer-events-none",
  outerAnimatedBand: "absolute -top-24 -right-24 w-72 h-72 rounded-full bg-[#0E7C66]/[0.06] blur-0",
  innerWhiteBorder: "absolute top-6 left-6 w-10 h-10 border-t-2 border-l-2 border-[#0E7C66]/30 rounded-tl-2xl pointer-events-none",

  // Content container
  contentContainer: "relative z-20 p-6 sm:p-8 md:p-10 lg:p-12",

  // Layout styles
  flexContainer: "flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12",
  leftContent: "flex-1 text-center lg:text-left",
  rightImageSection: "flex-1 relative w-full",

  // Header with badge
  headerBadgeContainer: "flex flex-col lg:flex-row items-center justify-center lg:justify-start mb-3 lg:mb-4 gap-4",
stethoscopeContainer: "relative shrink-0",
stethoscopeInner: "relative bg-[#EFF7F3] p-3 rounded-2xl ring-1 ring-[#0E7C66]/20 shadow-[0_6px_14px_-8px_rgba(14,124,102,0.35)]",
stethoscopeIcon: "w-6 h-6 text-[#0E7C66]",

  // Title — one serif family throughout (the script "pacifico" font in the
  // old version fought the serif logotype in the actual screenshot).
  // "Medi" carries the ink color, "xthon+" carries the brand teal —
  // same weight, same face, just a color break to mark the product name.
  titleContainer: "font-serif",
  title: "text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-[#0B2B22] mb-1 tracking-tight",
  titleGradient: "text-[#0E7C66]",

  // Stars
  starsContainer: "flex items-center justify-center lg:justify-start mt-1.5 gap-2",
  starsInner: "flex gap-0.5",
  starIcon: "w-4 h-4 fill-[#E8A93B] text-[#E8A93B]",

  // Tagline
  tagline: "text-lg sm:text-xl md:text-2xl font-light text-[#4A5A54] mb-2 leading-snug",
  taglineHighlight: "text-[#0E7C66] font-semibold",

  // Signature element — a thin ECG pulse-line divider between the tagline
  // and the feature list. Render as inline SVG using this class; the one
  // deliberately "medical" flourish in the design, used exactly once.
  pulseDivider: "w-28 h-5 mb-5 mx-auto lg:mx-0 text-[#0E7C66]/60",

  // Features grid — pills are outlined + tinted rather than solid-filled,
  // so the eye lands on the icon and label instead of four identical
  // green rectangles. "Safe & Secure" gets the deep-fill treatment since
  // it doubled as the visually "selected" state in the original screenshot.
  featuresGrid: "grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-7 text-sm sm:text-[15px]",
  featureItem: "flex items-center justify-center lg:justify-start bg-[#EFF7F3] px-4 py-3 rounded-2xl border border-[#0E7C66]/15 transition-colors duration-200 hover:bg-[#E1F1EA]",
  featureItemEmphasis: "flex items-center justify-center lg:justify-start bg-[#0B2B22] px-4 py-3 rounded-2xl border border-[#0B2B22] transition-colors duration-200",
  featureIcon: "w-4 h-4 text-[#0E7C66] mr-3 shrink-0",
  featureIconEmphasis: "w-4 h-4 text-[#5DCAA5] mr-3 shrink-0",
  featureText: "text-[#0B2B22] font-medium",
  featureTextEmphasis: "text-white font-medium",

  // Feature border colors — retained as keys, remapped to the restrained
  // two-tone palette (teal for info, none of the rainbow blue/purple/pink
  // the old version implied it would cycle through).
  featureBorderGreen: "border-[#0E7C66]/15",
  featureBorderBlue: "border-[#0E7C66]/15",
  featureBorderEmerald: "border-[#0B2B22]",
  featureBorderPurple: "border-[#0E7C66]/15",

  // CTA Buttons container
  ctaButtonsContainer: "flex flex-col sm:flex-row gap-3 justify-center lg:justify-start",

  // Book appointment button — solid ink-teal, not a lighter-toward-white
  // gradient (which was low-contrast against a white card).
  bookButton: "group relative lg:whitespace-nowrap bg-linear-to-r from-green-500 to-sky-400 text-white px-7 py-3.5 rounded-full font-semibold transition-all duration-300 shadow-[0_10px_20px_-8px_rgba(14,124,102,0.55)] hover:shadow-[0_14px_24px_-8px_rgba(14,124,102,0.65)] hover:bg-[#0A5C4C] overflow-hidden text-sm sm:text-base",
  bookButtonOverlay: "absolute inset-0 bg-white/15 transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700",
  bookButtonContent: "relative flex items-center justify-center gap-2",
  bookButtonIcon: "w-4 h-4 sm:w-5 sm:h-5",

  // Emergency button — outline-first so it reads as "available" rather
  // than a second competing solid CTA; fills on hover for urgency.
  emergencyButton: "group border-2 lg:whitespace-nowrap border-[#E2574C] text-[#E2574C] bg-[#FBEAE8] px-7 py-3.5 rounded-full font-semibold transition-all duration-300 hover:bg-[#E2574C] hover:text-white text-sm sm:text-base",
  ...
  imageFrame: "relative overflow-hidden rounded-[20px] ring-1 ring-[#0B2B22]/10 shadow-[0_24px_48px_-20px_rgba(11,43,34,0.35)]",
  image: "w-full object-cover h-56 sm:h-72 md:h-96 lg:h-[360px] xl:h-[420px] transition-transform duration-700",

  // New: floating credential card that sits on the image corner instead
  // of relying only on the "500+ Doctors" pill in the text column.
  imageBadge: "absolute -bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-56 bg-white rounded-2xl shadow-[0_16px_32px_-12px_rgba(11,43,34,0.35)] border border-[#0E7C66]/10 px-4 py-3 flex items-center gap-3",
  imageBadgeIconWrap: "w-9 h-9 rounded-full bg-[#EFF7F3] flex items-center justify-center shrink-0",
  imageBadgeIcon: "w-5 h-5 text-[#0E7C66]",
  imageBadgeTitle: "text-[#0B2B22] font-semibold text-sm leading-tight",
  imageBadgeSubtitle: "text-[#4A5A54] text-xs",

  // Carousel — auto-scrolling image strip inside the same frame.
  carouselViewport: "relative w-full h-56 sm:h-72 md:h-96 lg:h-[360px] xl:h-[420px] overflow-hidden",
  carouselTrack: "flex h-full transition-transform duration-700 ease-out",
  carouselSlide: "w-full h-full shrink-0 object-cover",
  // soft dark gradient at the bottom of the frame — gives the dots/arrows
  // a readable surface and adds depth so the photo doesn't look like a
  // flat pasted rectangle.
  carouselFade: "pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-black/45 to-transparent",
  carouselArrow: "absolute top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-white/95 text-[#0B2B22] flex items-center justify-center shadow-[0_4px_14px_rgba(11,43,34,0.28)] hover:bg-white hover:scale-105 active:scale-95 transition-all duration-200",
  carouselArrowLeft: "left-3",
  carouselArrowRight: "right-3",
  carouselDots: "absolute bottom-3 left-1/2 -translate-x-1/2 z-10 flex gap-1.5",
  carouselDot: "w-1.5 h-1.5 rounded-full bg-white/60 transition-all duration-300 hover:bg-white/80",
  carouselDotActive: "w-4 h-1.5 rounded-full bg-white",
};
...
export const footerStyles = {
  ...
  ============================ */
footerContainer:
"relative overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-cyan-50 text-zinc-900 border-t border-emerald-200",

  floatingIcon1:
    "absolute top-10 right-10 hidden xl:block animate-float opacity-10",

  floatingIcon2:
    "absolute left-10 bottom-24 hidden xl:block animate-float opacity-10",
  ...
  mainContent:
    "relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-16",

  gridContainer:
    "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-12",
  ...
  logoContainer:
    "flex items-center gap-4 mb-7 group",

  logoWrapper:
    "relative",

  logoImageContainer:
    "w-20 h-20 rounded-3xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl shadow-2xl flex items-center justify-center transition-all duration-500 group-hover:rotate-6 group-hover:scale-105",

  logoImage:
    "w-14 h-14 object-contain",

  companyName:
"text-3xl font-bold tracking-tight bg-gradient-to-r from-emerald-700 via-teal-600 to-cyan-600 bg-clip-text text-transparent",

 companyTagline:
"text-sm tracking-widest uppercase text-emerald-600 mt-1",

 companyDescription:
"mt-3 text-zinc-700 leading-7 text-[15px]",
  ...
  contactItem:
    "flex items-center gap-4 transition-all duration-300 hover:translate-x-2",

  contactIconWrapper:
"w-11 h-11 rounded-2xl bg-white shadow-lg border border-emerald-100 flex items-center justify-center transition-all duration-300 hover:shadow-xl hover:-translate-y-1",
 contactIcon:
...
sectionTitle:
"text-xl font-bold text-zinc-900 mb-7 relative inline-block after:absolute after:-bottom-2 after:left-0 after:w-14 after:h-1 after:bg-gradient-to-r after:from-emerald-500 after:to-cyan-500 after:rounded-full",
  linksList:
    "space-y-2",
 ...
 quickLink:
"group flex items-center gap-3 text-zinc-700 py-2.5 px-3 rounded-xl transition-all duration-300 hover:bg-emerald-100 hover:text-emerald-700 hover:translate-x-2",
  quickLinkIconWrapper:
"w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center transition-all duration-300 group-hover:bg-emerald-500 group-hover:rotate-12",
quickLinkIcon:
"w-4 h-4 text-emerald-600 group-hover:text-white",
  /* ============================
          Services
  ============================ */
...
"group flex items-center gap-3 text-zinc-700 py-2.5 px-3 rounded-xl transition-all duration-300 hover:bg-emerald-100 hover:text-emerald-700 hover:translate-x-2",
  serviceIcon:
    "w-2.5 h-2.5 rounded-full bg-gradient-to-r from-emerald-400 to-cyan-400",
...
"text-zinc-700 mt-3 leading-7 text-sm",
  ...
  mobileNewsletterContainer:
    "flex flex-col gap-4 lg:hidden mt-3",

 emailInput:
"w-full rounded-2xl border border-emerald-200 bg-white px-5 py-3 text-zinc-800 placeholder:text-zinc-400 outline-none transition-all duration-300 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-200 shadow-md",
  mobileSubscribeButton:
    "flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-500 to-cyan-500 py-3 text-white font-semibold shadow-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-emerald-500/40",
  ...
  desktopNewsletterContainer:
    "relative hidden lg:block mt-5",

 desktopEmailInput:
"w-full rounded-2xl border border-emerald-200 bg-white py-4 pl-6 pr-36 text-zinc-800 placeholder:text-zinc-400 outline-none transition-all duration-300 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-200 shadow-md",
  desktopSubscribeButton:
    "absolute right-2 top-2 bottom-2 px-5 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 flex items-center justify-center text-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-emerald-500/40",
  ...
  socialContainer:
    "flex flex-wrap gap-4 mt-8 justify-center lg:justify-start",

  socialLink:
    "group relative",

  socialIconBackground:
    "absolute inset-0 rounded-2xl bg-gradient-to-br from-emerald-400/30 to-cyan-400/30 blur-xl opacity-0 group-hover:opacity-100 transition-all duration-500",

 socialIcon:
"relative z-10 w-12 h-12 rounded-2xl bg-white border border-emerald-100 p-3 text-zinc-600 transition-all duration-500 hover:-translate-y-2 hover:rotate-6 hover:shadow-xl",
  facebookColor:
    "hover:text-blue-500",
...
bottomSection:
"mt-16 pt-8 border-t border-emerald-200 flex flex-col md:flex-row items-center justify-between gap-4",
 copyright:
"text-zinc-600 text-sm",

  designerText:
"text-zinc-600 text-sm flex items-center gap-2",
  designerLink:
"font-semibold text-emerald-600 hover:text-emerald-800 transition-all duration-300",
  /* ============================
        Animation
  ============================ */
...
loginButton:
"hidden lg:inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-gradient-to-r from-[#10B981] to-[#059669] text-white text-sm font-semibold shadow-lg hover:shadow-emerald-300/40 hover:-translate-y-0.5 hover:from-[#059669] hover:to-[#047857] transition-all duration-300 ease-in-out active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 cursor-pointer",  doctorAdminText: "hidden lg:text-xs lg:whitespace-nowrap sm:inline-block",
  loginIcon: "w-7 h-4",
...
export const testimonialStyles = {
  ...
  title: "text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-zinc-500 via-green-900 to-green-500 mb-3",
